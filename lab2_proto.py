...
def concatTwoHMMs(hmm1, hmm2):
    """ Concatenates 2 HMM models, Love's version
    Args:
       hmm1, hmm2: two dictionaries with the following keys:
           name: phonetic or word symbol corresponding to the model
           startprob: M+1 array with priori probability of state
    ...
    startprob = np.concatenate(
        [hmm1["startprob"][0:-1], hmm1["startprob"][-1] * hmm2["startprob"]]
    )

    top_right = (
        hmm1["transmat"][:, -1][:, np.newaxis] @ hmm2["startprob"].reshape(1, -1)
    )[0:-1, :]

    top_left = hmm1["transmat"][0:-1:, 0:-1]

    bottom_right = hmm2["transmat"]

    return {
        "startprob": startprob,
        "transmat": np.block(
            [[top_left, top_right], [np.zeros(top_right.T.shape), bottom_right]]
        ),
...
def viterbi(log_emission_lik, log_startprob, log_transmat, forceFinalState=True):
    """Viterbi path., Love's version
    Args:
